...
public class InvoiceReport {
	...
	public static void main(String[] args) {
		...
		try {
			List<Person> people = DataLoader.loadPeople(personsFile);
			List<Item> items = DataLoader.loadItems(itemsFile);
			List<Company> companies = DataLoader.loadCompanies(companiesFile, people);
			List<Invoice> invoices = DataLoader.loadInvoices(invoicesFile, invoiceItemsFile, people, companies, items);

			invoices.sort(Comparator.comparing(Invoice::getGrandTotal).reversed());
			...
			System.err.println("ERROR: Could not load data or generate reports");
			e.printStackTrace();
		}
	}
	...
	private static void createTotalSummary(List<Invoice> invoices) {
		...
		System.out.println(
				"----------------------------------------------------------------------------------------------------------");
		System.out.printf("%-36s %-32s %-12s %-11s %-11s%n", "Invoice #", "Customer", "Num Items", "Tax", "Total");
		...
		for (Invoice invoice : invoices) {
			...
			System.out.printf("%-36s %-32s %-12d $%10.2f $%10.2f%n", invCode, compName, numItems, tax, grand);
		}

		System.out.println(
				"----------------------------------------------------------------------------------------------------------");
		System.out.printf("%-69s %-12d $%10.2f $%10.2f%n%n", "", totalItems, totalTax, totalGrandTotal);
	}
	...
	private static void createCompanySummary(List<Company> allCompanies, List<Invoice> invoices) {
		...
		System.out.println(
				"----------------------------------------------------------------------------------------------------------");
		System.out.printf("%-80s %-12s %-12s%n", "Company", "# Invoices", "Grand Total");
		...
		for (String compName : sortedCompanyNames) {
			...
			System.out.printf("%-80s %-12d $%11.2f%n", compName, currentCount, currentTotal);
		...
		System.out.println(
				"----------------------------------------------------------------------------------------------------------");
		System.out.printf("%-80s %-12d $%11.2f%n%n", "", totalInvoices, overallGrandTotal);
	}
	 ...
	 * @param invoices
	 * 
	 * ALL SPACING MUST EQUAL 106 FOR FORMATTING TODO: delete
	 */
	private static void createReceipt(List<Invoice> invoices) {
		for (Invoice invoice : invoices) {
			...
			System.out.printf("%-82s %-12s %-12s%n", "Items (" + invoice.getItems().size() + ")", "Tax", "Total");
			System.out.println(
					"----------------------------------------------------------------------------------  ----------  ----------");

			for (InvoiceItem item : invoice.getItems()) {
				String[] itemLines = item.toString().split("\n");
				System.out.printf("%-82s $%10.2f $%10.2f%n", itemLines[0], item.getTax(), item.getCost());
			...
			System.out.println(
					"                                                                                    ----------  ----------");
			System.out.printf("%-82s $%10.2f $%10.2f%n",
					"                                                                         Subtotals",
					invoice.getTaxTotal(), invoice.getSubtotal());
			System.out.printf("%-82s %-12s$ %9.2f%n",
					"                                                                       Grand Total", "",
